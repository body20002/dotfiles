# source: https://github.com/qtile/qtile-examples/blob/master/floating_window_snapping.py
...
def _get_borders(window):
    ...
    for s in window.qtile.screens:
        ...
        borders.extend(
            (w.x, w.y, w.x + w.width + 2 * w.borderwidth, w.y + w.height + 2 * w.borderwidth)
    ...
    borders.remove(
        (
            window.x,
            window.y,
            window.x + window.width + 2 * window.borderwidth,
            window.y + window.height + 2 * window.borderwidth,
        )
    )
    return borders
...
def _borders_touch(window, x, y, snap_dist):
    ...
    for b in borders:
        ...
        if (
            any(i in [window.edges[0], window.edges[2] + 2 * window.borderwidth] for i in [b[0], b[2]])
            and window.edges[1] < b[3]
            and window.edges[3] > b[1]
            and any(abs(window.edges[i] - x) < snap_dist for i in [0, 2])
        ):
        ...
        # Repeat for y
        if (
            any(i in [window.edges[1], window.edges[3] + 2 * window.borderwidth] for i in [b[1], b[3]])
            and window.edges[0] < b[2]
            and window.edges[2] > b[0]
            and any(abs(window.edges[i] - y) < snap_dist for i in [1, 3])
        ):
            with contextlib.suppress(Exception):
                del overlap_args["y"]
    return overlap_args
